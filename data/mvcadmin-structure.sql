...
CREATE UNIQUE INDEX `theuserlogin_UNIQUE` ON `mvcadmin`.`theuser` (`theuserlogin` ASC) VISIBLE;

CREATE INDEX `fk_theuser_theright_idx` ON `mvcadmin`.`theuser` (`theright_idtheright` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `mvcadmin`.`thearticle` (
  ...
  `thearticleStatut` TINYINT NOT NULL DEFAULT 1 COMMENT '0 => wait for validation\n1 => validate\n2 => wait for correction\n3 => banned',
...
CREATE INDEX `fk_thearticle_theuser1_idx` ON `mvcadmin`.`thearticle` (`theuser_idtheuser` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `mvcadmin`.`thesection` (
  ...
  `thesectionTitle` VARCHAR(45) NOT NULL,
  `thesectionDesc` VARCHAR(500) NULL,
  PRIMARY KEY (`idthesection`))
ENGINE = InnoDB;

CREATE UNIQUE INDEX `thesectionTitle_UNIQUE` ON `mvcadmin`.`thesection` (`thesectionTitle` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `mvcadmin`.`thesection_has_thearticle`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mvcadmin`.`thesection_has_thearticle` ;

CREATE TABLE IF NOT EXISTS `mvcadmin`.`thesection_has_thearticle` (
...
CREATE INDEX `fk_thesection_has_thearticle_thearticle1_idx` ON `mvcadmin`.`thesection_has_thearticle` (`thearticle_idthearticle` ASC) VISIBLE;

CREATE INDEX `fk_thesection_has_thearticle_thesection1_idx` ON `mvcadmin`.`thesection_has_thearticle` (`thesection_idthesection` ASC) VISIBLE;
